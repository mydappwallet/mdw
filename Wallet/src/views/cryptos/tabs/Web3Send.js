...
handleMaxAmount() {
  ...
  handleAmountChange(amount) {
   const exchangeRates = this.props.appStore.exchangeRates[this.crypto.symbol];
...
onSubmited(error, result){
   ...
   if(this.state.redirect)return <Redirect to={this.state.redirect}/>	
      ...
      const {gasPrice, symbol} = this.crypto;
	  const exchangeRates = this.props.appStore.exchangeRates[symbol];
  ...
  <Form onSubmit={handleSubmit} noValidate name='addForm'>
			  ...
			  <FormGroup>
                <label htmlFor="asset" className="highlight"><Translate id="from"/>	&nbsp;</label>
     			<InputGroup className="mb-3 asset-group">
	...
	<div className="asset">
        ...
        <Row>
				 ...
				 <Col style={{maxWidth: 450}}>
			  <FormGroup>
         <label htmlFor="to" className="highlight"><Translate id="to"/></label>
      ...
      />  
       ...
       <Row>
          	<Col style={{maxWidth: 450}}>
			  <FormGroup>
                <label htmlFor="amount" className="highlight"><Translate id="amount"/>	&nbsp;
<div class="send-v2__amount-max">
				...
				<div  className={'amount-input'+ (!errors.amount?" is-valid":(touched.amount && !!errors.amount)?" is-invalid":"")}>
				 ...
				 <NumberFormat  className='amount-input__input bold' 
					                     name="amount"
					                     id="amount"
					                    autoFocus={true}
					                     required
...
{ gasPrice && <React.Fragment>
    ...
    <Collapse open={!this.state.advancedOptions}>
     ...
     <Collapse open={this.state.advancedOptions || !gasPrice }>
    ...
    <Row>
	 		...
	 		<Col style={{maxWidth: 225}}>
			  <FormGroup>
                <label htmlFor="gasPrice" className="highlight"><Translate id="gasPrice"/></label>
	                      ...
	                      onChange={ (e)=> {handleChange(e); this.handleGasPriceChange(e.target.value); }}
										 ...
										 disabled={this.state.submit}
                     className={"form-control".concat(" high ").concat((!!errors.gasPrice)?"is-invalid":"is-valid") }
		...
		<Col style={{maxWidth: 225}}>
			  <FormGroup>
                <label htmlFor="gasLimit" className="highlight"><Translate id="gasLimit"/></label>
                <NumberFormat 
					                     name="gasLimit"
					                     id="gasLimit"
					                    autoFocus={true}
										 ...
										 disabled={this.state.submit}
                     className={"form-control".concat(" high ").concat((!!errors.gasLimit)?"is-invalid":"is-valid") }
